...
in general, an NP-hard problem; we avoid solving it directly by a
...
solutions, matching them to within $5.6\times10^{-12}$, and pass 36
automated checks spanning analytical benchmarks, exhaustive enumeration,
brute-force search, and randomised sampling. On a graph with
$1.68\times10^{7}$ possible routes, the route oracle recovers the known
closed-form value while evaluating only two of those routes, although on
the widest instances the outer cutting-plane certificate does not close
within our iteration limit. We also show that uniform allocation of the
budget -- reported as empirically good in earlier reliability studies --
is exactly optimal for the symmetric graph families we analyse, but
$211\%$ worse than optimal on average over 20 irregular graphs, and that
a "defend the most central node" rule can fail completely.
...
\item \textbf{A partial strict-convexity result, and an open question.}
  ...
  route attains the maximum} (Proposition~\ref{prop:partial-strict}). This
  falls short of strict convexity in the full allocation vector, because
  $g_P$ does not depend on nodes off $P$. Uniqueness of the optimal
  allocation therefore remains open; we state precisely what is missing
  (Remark~\ref{rem:uniqueness-open}) and report numerical evidence for it
...
\item \textbf{An algorithm that avoids enumerating routes.} Finding the
  evader's best route is a maximum-reliability path problem, shown
  NP-hard by \citet{nguyen2023}. We give a relaxation that reduces this
  search to one shortest-path computation, plus a rule that certifies when
  no unexamined route can beat the current best
...
(Section~\ref{sec:oracle}). Their proof that the underlying path problem
is NP-hard is why we do not look for a polynomial exact algorithm.
...
This search covered the two papers that motivated the project plus
targeted searches on Tullock contests on networks, sequential
interdiction with ratio-form contests, and network extensions of the
Colonel Blotto game. It is a keyword search, not a systematic citation
...
\begin{remark}[Why this does not yet give uniqueness]
...
Closing this would require showing that two distinct minimisers cannot
differ only at nodes off every route that is active at their midpoint.
...
set of nodes meeting \emph{every} active route, which we have not carried
out. We therefore state uniqueness as an open question rather than a
theorem. Section~\ref{sec:results} reports numerical evidence consistent
with uniqueness on the instances we tested; that is evidence, not proof.
\end{remark}
...
Remark~\ref{rem:uniqueness-open} leaves uniqueness of the optimal
allocation open. We report the relevant numerical evidence here, and we
are explicit that it is evidence and not a substitute for a proof.

The solver is run from three very different starting allocations -- an
even split, a random allocation, and one concentrated in a single corner
of the simplex -- on four graphs (the worked example, the diamond, the
unequal-branches graph, and a $4\times4$ grid), and the resulting
$x^{\star}$ is compared across runs. Across all four graphs and all three
starting points, the largest coordinate-wise difference in the final
allocation is $2.0\times10^{-5}$, at a solver tolerance of $10^{-10}$ in
log-value. On these instances the solver therefore reaches the same
allocation regardless of where it starts.

This is consistent with a unique minimiser, but it does not establish
one: a convex problem with a genuinely flat set of minimisers could still
produce repeatable output from a deterministic algorithm, since nothing
in the method forces it to explore the flat directions. Settling the
question requires the argument sketched in
Remark~\ref{rem:uniqueness-open}, not more runs.
...
\item[Uniqueness of the optimal allocation is not proved.] We establish
  only partial strict convexity (Proposition~\ref{prop:partial-strict}),
  which is not enough to rule out a flat set of minimisers; see
  Remark~\ref{rem:uniqueness-open} for exactly what is missing. The
  numerical evidence in Section~\ref{sec:results-uniqueness} is
  consistent with uniqueness but does not establish it.
